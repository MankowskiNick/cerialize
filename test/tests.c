...
int main() {
    ...
    agg_rows[6] = get_aggregate_output_row("Total", total_passed, total_failed, total_tests);

    const char *agg_headers[] = {"Type", "Passed", "Failed", "Total"};
    int agg_col_widths[] = {10, 8, 8, 8};
    print_test_table("Aggregate Test Results", agg_headers, 4, agg_col_widths, agg_rows, 7);
